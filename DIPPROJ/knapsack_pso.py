...
items = ['Televizyon', 'Kamera', 'Projektör', 'Walkman', 'Radyo', 'Cep Telefonu', 'Dizüstü Bilgisayar']
...
class PSO:
    ...
    def SonucuYazdir(self):
        ...
        for i in range(len(self.grupMaxPozisyon)):
            print(items[i], ': ', self.grupMaxPozisyon[i], ' tane', sep='')
            toplamKar += self.grupMaxPozisyon[i] * kar[i]
            toplamKg += self.grupMaxPozisyon[i] * kg[i]
...
for i in range(len(items)):
    ...
    print(items[i], ': ', sinirDegerler[i][0], ' - ', sinirDegerler[i][1], sep='')
print('\nolmak üzere toplam ', len(items), ' değişken var...\n\n', sep='')

pso = PSO(fncMax, baslangicDegerleri, sinirDegerler, parcacikSayisi=len(items), suruSayisi=100, maxIter=50, adimlarYazdirilsinMi=True)
pso.SonucuYazdir()
